...
db.getCollection('pokedex').insertMany(
    [{
        "id": 1,
        "name": {
          "english": "Bulbasaur",
...
//Create in MongoDB a collection called pokedex.

//Insert all pokemons in the pokemon collection (one document for each pokemon).

//Search all the characteristics related with the pokemon named Squirtle.
    //db.pokedex.find({ "name.english": "Squirtle" });
...
//Show a list with the english name of all Pokemons with attack 20 or lower.
    ...
    //db.pokedex.find({"base.Attack":{$lte: 20}},{"name.english":1,"_id":0});

//Show a list with the english name of all Pokemons with attack 50 or greater and type Poison.

    //db.pokedex.find({"base.Attack": {$gte:50},"type":"Poison"},{"name.english":1,"_id":0});
...
/*
    db.pokedex.updateMany(
        {
            "name.english":{$in:["Pikachu","Squirtle","Bulbasaur","Charmander"]}
        },
    {$set: {"base.Attack":{$add:["$strong Attack","$ultra Attack"]}}})
*/
...
db.pokedex.find({type:{$ne:"Bug"}},{"name.french":1,"_id":0});
